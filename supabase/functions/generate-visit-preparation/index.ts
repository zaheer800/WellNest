...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are preparing a patient for their doctor visit. Based on the following health data, generate a visit preparation guide.

Return JSON with exactly these fields:
- reports_to_carry: array of { name, date, is_new }
- symptoms_to_mention: array of { symptom, onset, severity, notes }
- questions_to_ask: array of strings
- what_doctor_will_check: array of strings
- medications_to_discuss: array of { medication, concern }

Health data:
${JSON.stringify({ appointment, recentReports, symptoms, medications, restrictions })}`
    ...
    const visitPreparation = JSON.parse(jsonMatch[1])
