            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide leading-none mb-0.5">
                  Dr. {doctorName} · {specialtyLabel}
                </p>
                <h1 className="text-base font-bold text-gray-900 leading-tight">{patientName}</h1>
              </div>
            </div>
            <button
                      ...
                      {params.map((p) => (
                        <div key={p.id} className="py-2 border-b border-gray-50 last:border-0">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-sm font-medium text-gray-800 flex-1">{p.parameter_name}</p>
                          ...
                          {p.reference_min != null && p.reference_max != null && (
                            <p className="text-xs text-gray-400 mt-0.5">Ref: {p.reference_min}–{p.reference_max} {p.unit}</p>
                          )}
                          {p.plain_language_explanation && (
                            <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">{p.plain_language_explanation}</p>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                symptomLogs.map((s) => (
                  <div key={s.id} className="flex items-start gap-3 py-2.5 border-b border-gray-50 last:border-0">
                    <div className="flex-1 min-w-0">
                      ...
                      {s.notes && <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">{s.notes}</p>}
                    </div>
              ...
              <div className="p-4 border-b border-gray-50 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white"
                  >
                  ...
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center bg-teal-600 text-white rounded-xl disabled:opacity-50 transition flex-shrink-0"
                    aria-label="Add note"
                  >
...
function ImagingReportRow({ report, patientId, onDownload }: { report: any; patientId: string; onDownload: (path: string, label: string) => void }) {
  ...
  return (
    <div className="border-b border-gray-50 last:border-0">
      <button className="w-full py-3 text-left active:bg-gray-50 transition rounded-xl" onClick={handleExpand}>
        <div className="flex items-start gap-2">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {report.surgical_urgency && <span className="text-xs text-red-600 font-semibold">Urgent</span>}
            </div>
          </div>
          {expanded
...
function Section({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-2 px-4 py-3.5 hover:bg-gray-50 active:bg-gray-50 transition"
